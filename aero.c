 ...
 *
 * Welcome to aero! aero is a window manager built atop dwm (dynamic window
 * manager).
 *
...
void clientmessage(XEvent *e) {
  ...
  // ABSOLUTE BEST BUG FIX OF THE CENTURY
  if (!c)
    return;
...
void focusstack(const Arg *arg) {
  ...
  if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
    return;
  if (arg->i > 0) {
    for (c = selmon->sel->next; c && (!ISVISIBLE(c) || c->cantfocus);
         c = c->next)
      ;
    if (!c)
      for (c = selmon->clients; c && (!ISVISIBLE(c) || c->cantfocus);
           c = c->next)
        ;
  } else {
...
XKeyEvent createkeyevent(Display *display, Window win, Window rootWindow,
                         int type, KeyBinding *keyBinding) {
...
void sendkeyevent(const Arg *arg) {
  ...
  XKeyEvent event =
      createkeyevent(dpy, focusedWindow, rootWindow, KeyPress, keyBinding);
  XSendEvent(event.display, event.window, True, KeyPressMask, (XEvent *)&event);

  event =
      createkeyevent(dpy, focusedWindow, rootWindow, KeyRelease, keyBinding);
...
void setup(void) {
  ...
  XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
                  PropModeReplace, (unsigned char *)"aero", 4);
...
void spawn(const Arg *arg) {
  ...
  if (fork() == 0) {
    ...
    die("aero: execvp '%s' failed:", ((char **)arg->v)[0]);
  }
}
